...
impl<
    ...
    + MaybeStorageLayoutTrait
    > Q<A, B> 
    where Precision<A>: PrecisionTrait {
    ...
    pub fn new_from_int<
        ...
        + MaybeTypeInfoTrait
        + MaybeStorageLayoutTrait
        >(v: D) -> QTraitResult<Self> 
        where Precision<C>: PrecisionTrait {
        let v: B = B::from(v).ok_or(QTraitError::ConversionFailure)?;
        let v: Q<C, B> = q(v);
        let v: Q<A, B> = v.cast()?;
        Ok(v)
...
impl<const A: u8, B> PartialOrdTrait for Q<A, B> where
    B: PrimIntTrait,
    B: BrandedTrait,
    B: MaybeEncodeTrait,
    B: MaybeDecodeTrait,
...
impl<const A: u8, B> OrdTrait for Q<A, B> where 
    B: PrimIntTrait,
    B: BrandedTrait,
    B: MaybeEncodeTrait,
    B: MaybeDecodeTrait,
...
#[cfg(not(feature = "ink"))]
pub fn q_fl<const A: u8, B: FloatTrait, const C: u8, D: PrimIntTrait + BrandedTrait + SignIntrospectionTrait>(v: B) -> QTraitResult<Q<C, D>> where
    Precision<A>: PrecisionTrait,
    Precision<C>: PrecisionTrait, {
    Q::from_float::<A, B>(v)
}
