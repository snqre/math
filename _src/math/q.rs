...
where
    ...
    fn sqrt<const A: u8, B>(&self, x: &Q<A, B, Self>) -> QR<Q<A, B, Self>>
    where
        B: int::Int,
        B: int::Introspection {
        ...
        let x: B = x._v;
        if x.is_signed() {
            let x: i128 = x.to_i128().ok_or(QE::UnsupportedConversion)?;
            if x < 0 {
                return Err(QE::NegSqrt)
            }
            let x: u128 = x.to_u128().ok_or(QE::UnsupportedConversion)?;
            let x: u128 = x * scale;
            let x: u128 = _sqrt(x);
            let engine: Self = self.clone();
            let result: B = B::from(x).ok_or(QE::UnsupportedConversion)?;
    ...
    fn cast<const A: u8, const B: u8, C>(&self, x: &Q<A, C, Self>) -> QR<Q<B, C, Self>> 
    where
        C: int::Int,
        C: int::Introspection {
        let x: C = x._v;
    ...
    fn mul<const A: u8, B>(&self, x: &Q<A, B, Self>, y: &Q<A, B, Self>) -> QR<Q<A, B, Self>>
    where
        B: int::Int,
        B: int::Introspection {
        let x: B = x._v;
    ...
    fn div<const A: u8, B>(&self, x: &Q<A, B, Self>, y: &Q<A, B, Self>) -> QR<Q<A, B, Self>>
    where
        B: int::Int,
        B: int::Introspection {
        let x: B = x._v;
        ...
        if x.is_signed() {
            ...
            if scale.is_power_of_two() {
                ...
                let engine: Self = self.clone();
                let r: i128 = (x << z).checked_div(y).ok_or(QE::DivByZero)?;
                let r: B = B::from(r).unwrap();
                let r: Q<A, B, Self> = new_with_custom_engine(r, engine);
                return r.into_ok()
            }
        ...
        if scale.is_power_of_two() {
            let z: u32 = scale.trailing_zeros();
            let engine: Self = self.clone();
            let r: u128 = (x << z).checked_div(y).ok_or(QE::DivByZero)?;
            let r: B = B::from(r).unwrap();
            let r: Q<A, B, Self> = new_with_custom_engine(r, engine);
            return r.into_ok()
        }
    ...
    fn rem<const A: u8, B>(&self, x: &Q<A, B, Self>, y: &Q<A, B, Self>) -> QR<Q<A, B, Self>> 
    where
        B: int::Int,
        B: int::Introspection,
        B: num::CheckedRem {
        ...
        let y: B = y._v;
        if y == B::zero() {
            return QE::DivByZero.into_err()
        }
        let r: B = x.checked_rem(&y).ok_or(QE::RemByZero)?;
        let engine: Self = self.clone();
        let r: Q<A, B, Self> = new_with_custom_engine(r, engine);
        r.into_ok()
    }

    fn add<const A: u8, B>(&self, x: &Q<A, B, Self>, y: &Q<A, B, Self>) -> QR<Q<A, B, Self>> 
    where 
        B: int::Int,
        B: int::Introspection {
        let x: &Q<A, B, Self> = x;
        ...
        let y: &Q<A, B, Self> = y;
        let y: B = y._v;
        let z: B = x.checked_add(&y).ok_or(QE::Overflow)?;
        let engine: Self = self.clone();
        let z: Q<A, B, Self> = new_with_custom_engine(z, engine);
        z.into_ok()
    }

    fn sub<const A: u8, B>(&self, x: &Q<A, B, Self>, y: &Q<A, B, Self>) -> QR<Q<A, B, Self>>
    where
        B: int::Int,
        B: int::Introspection {
        let x: &Q<A, B, Self> = x;
        ...
        let y: &Q<A, B, Self> = y;
        let y: B = y._v;
        let z: B = x.checked_sub(&y).ok_or(QE::Overflow)?;
        let engine: Self = self.clone();
        let z: Q<A, B, Self> = new_with_custom_engine(z, engine);
        z.into_ok()
    }

    fn muldiv<'a, T>(&self, x: &'a T, y: &'a T, z: &'a T) -> QR<T>
    where
        T: int::Int,
        T: int::Introspection {
        if z == &T::zero() {
        ...
        if z.is_signed() {
            ...
            if b >= z {
                return QE::Overflow.into_err()
            }
            if b == 0 {
                let r: u128 = a / z;
                if sign == 1 {
                    let r: T = r.to_i128()
                        .ok_or(QE::UnsupportedConversion)?
                        .wrapping_neg()
                        .to_int()
                        .ok_or(QE::UnsupportedConversion)?;
                    return r.into_ok()
                }
                let r: T = r.to_int().ok_or(QE::UnsupportedConversion)?;
                return r.into_ok()
            }
...
where
    ...
    pub fn to_degree(&self) {
        
    }
}

fn pi<const T: u8, B: int::Int + int::Introspection>() -> QR<Q<A, B, >> {
    let value: u128 = match T {
        1 => 3_1,
        2 => 3_14,
        3 => 3_141,
        4 => 3_1415,
    ...
    let unsupported_conversion_error: QE = QE::UnsupportedConversion;
    let value: B = B::from(value).ok_or(unsupported_conversion_error)?;
