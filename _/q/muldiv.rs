...
#[inline]
fn fold_signed<T: Int>(x: T, y: T, z: T) -> Result<T> {
    let (x, y, z) = unsafe {
...
#[inline]
fn fold_unsigned<T: Int>(x: T, y: T, z: T) -> Result<T> {
    let (x, y, z) = unsafe {
...
#[inline]
pub fn wide_mul<T: Int>(x: T, y: T) -> Result<(T, T)> {
    if T::IS_SIGNED {
...
#[inline]
fn wide_mul_signed<T: Int>(x: T, y: T) -> Result<(T, T)> {
    assert!(T::IS_SIGNED);
...
#[inline]
fn wide_mul_unsigned<T: Int>(x: T, y: T) -> Result<(T, T)> {
